...
const ROLE_TRANSITIONS: Record<
  BMADPhase,
  Record<AgentRole, { next: AgentRole; condition?: (work: any) => boolean }>
> = {
  [BMADPhase.BUILD]: {
    [AgentRole.ARCHITECT]: {
...
function determineNextPhase(currentPhase: BMADPhase, completedWork: any): BMADPhase {
  ...
  const phaseTransitions: Record<BMADPhase, () => BMADPhase> = {
    [BMADPhase.BUILD]: () => {
      ...
      if (
        completedWork.description.includes('completo') ||
    ...
    [BMADPhase.ANALYZE]: () => {
      ...
      if (
        completedWork.description.includes('análisis completo') ||
    ...
    [BMADPhase.DEPLOY]: () => {
      ...
      if (completedWork.description.includes('desplegado') || completedWork.description.includes('producción')) {
...
function generateHandoffInstructions(
  ...
  const handoffTemplates: Record<string, string> = {
    [`${AgentRole.ARCHITECT}-${AgentRole.DEVELOPER}`]: `El Arquitecto ha completado el diseño. Como Desarrollador, toma el PLAN.md y ARCH.md para implementar la solución. 
       Asegúrate de seguir los patrones arquitectónicos definidos y las decisiones técnicas documentadas.`,

    [`${AgentRole.DEVELOPER}-${AgentRole.TESTER}`]: `El Desarrollador ha completado la implementación. Como Tester, valida el código con pruebas exhaustivas. 
       Verifica la funcionalidad, rendimiento y cobertura de tests. Documenta cualquier issue encontrado.`,

    [`${AgentRole.TESTER}-${AgentRole.DEBUGGER}`]: `El Tester ha encontrado issues. Como Debugger, analiza los problemas reportados: ${completedWork.issues?.join(', ')}. 
       Encuentra la causa raíz, implementa correcciones y asegura que no se introduzcan regresiones.`,

    [`${AgentRole.DEBUGGER}-${AgentRole.TESTER}`]: `El Debugger ha aplicado correcciones. Como Tester, re-ejecuta las pruebas para validar los fixes. 
       Asegúrate de que los problemas anteriores están resueltos y no hay nuevas regresiones.`,

    [`${AgentRole.TESTER}-${AgentRole.ANALYST}`]: `Las pruebas están completas. Como Analyst, recopila y analiza todas las métricas del proyecto. 
       Genera insights sobre calidad, rendimiento y áreas de mejora.`,

    [`${AgentRole.ANALYST}-${AgentRole.ARCHITECT}`]: `El análisis está completo. Como Arquitecto, revisa las métricas y recomendaciones. 
       Determina si son necesarios ajustes arquitectónicos basados en los insights.`,

    [`${AgentRole.DEVELOPER}-${AgentRole.DEVOPS}`]: `El código está listo para despliegue. Como DevOps, prepara el entorno de producción, 
       configura el pipeline de despliegue y ejecuta el release siguiendo las mejores prácticas.`,

    [`${AgentRole.DEVOPS}-${AgentRole.TESTER}`]: `El despliegue está completo. Como Tester, ejecuta pruebas de aceptación en producción. 
 ...
 */
function getRequiredArtifacts(nextRole: AgentRole, _nextPhase: BMADPhase): string[] {
