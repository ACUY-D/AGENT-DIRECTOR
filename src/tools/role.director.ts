 ...
 */
function generateChecklist(role: AgentRole, _phase: BMADPhase, completedSteps: string[]): string[] {
  ...
  return checklist.map((item) => {
    ...
    const isCompleted = completedSteps.some((step) => step.toLowerCase().includes(itemKey.substring(0, 10)));
 ...
 */
function generateNextSteps(_role: AgentRole, phase: BMADPhase, _taskState: any): string[] {
  const steps: string[] = [];
